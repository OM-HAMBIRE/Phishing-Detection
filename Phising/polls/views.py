...
def index_func(request):
    ...
    if request.method == 'POST':
        ...
        if name != "":
            ...
            df2 = pd.DataFrame({'NumDots': [float(NumDots)], 'PathLevel': [float(PathLevel)],
                                'NumDash': [float(NumDash)],
                                'NumSensitiveWords': [float(NumSensitiveWords)],
                                'PctExtHyperlinks': [float(PctExtHyperlinks)],
                                'PctExtResourceUrls': [float(PctExtResourceUrls)],
                                'InsecureForms': [float(InsecureForms)],
                                'PctNullSelfRedirectHyperlinks': [float(tooLong)],
                                'FrequentDomainNameMismatch': [float(freq)],
                                'SubmitInfoToEmail': [float(SubmitInfoToEmail)],
                                'IframeOrFrame': [float(IframeOrFrame)]})
            pd.concat([df, df2])
